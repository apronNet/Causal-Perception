...
**Manually adjust as pairs in preview window** keeps the ordinary O1/O2 pair model but shows editable start-point handles and path arrows in the preview. **Manually adjust as individuals in preview window** replaces the pair controls with one card per ball.
